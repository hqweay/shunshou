...
Shunshou is a browser extension (Chrome / Edge) — and also a **userscript** that installs on desktop and mobile: it notices which page you have open and quietly swaps in the right actions — save books, movies, and games from Douban, collect videos and comments from Bilibili, turn X / Xiaohongshu / Instagram posts into cards in one click, send your selection to Orca Note / SiYuan. When you want to share something, a quote or a long article, a whole page of text and images is ready in a moment. Things you do often can even be set to "auto-run" — open the matching page and they run for you.
...
4. **Configure**: "**In-page config**" at the bottom of the orb opens right in the page — try fields / clips against the current page and pick article containers;
...
| Bilibili videos | Video info + comments (including replies): copy, generate a share card, save as a video note |
...
### Clean clips: article scope and site rules
...
- **One scope per action**: the action's expanded panel has "**Article scope**" to set it just for that action — pick containers (multiple blocks merge),
...
- **Share card**: select → a clean card; delivery is handled by "then do" steps — the out-of-the-box default is "Copy image", and you can add
...
- **Share full text / page clip**: the whole article and its images are laid out **in reading order** as a card, and very long ones split into several images with nothing cut off
  (by default the first is copied and the rest can be copied one by one / downloaded all from the result dialog — "Copy" just copies, it won't stuff files onto your disk);
