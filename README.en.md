...
- **Can step in, can step back**: per-site control — keep only the orb / only the selection capsule on a site, set it to "Quiet" (no auto-run, no suggestions), or turn it fully "Off" (no UI mounted, and it isn't counted in your browsing footprint either); one click on "This site" at the top of the orb panel toggles it, and you can resume anytime
...
One rule = a set of URL patterns + one tier; pick from five:

- **Normal**: the default — the toolbox is available on every site (nothing to configure)
- **Quiet**: the UI stays, but actions never auto-run and no suggestion dots appear
- **Dock only** / **Selection only**: keep one entry point and hide the other
- **Off**: nothing runs at all — no UI is mounted, and the visit isn't counted in your browsing footprint either (a refresh makes it fully take effect)
